...
api = Namespace('products', description='Neo4j product-related operations')
...
read_argparser = argparser.copy()
@api.route('/<string:target_beacon_id>/getbybeacon')
class Read(Resource):
    @api.expect(read_argparser)
    @api.doc(responses={
        200: 'Success',
    ...
    def get(self, target_beacon_id):
        '''Get a product by beacon'''
        try:
            args = read_argparser.parse_args()
            url = neo_http_ap + '/db/data/cypher'
            try:
                data = {'query': get_product_id_by_beacon, 'params': {'target_beacon_id': str(target_beacon_id)}}
                data = json.dumps(data, separators=(',', ':'))
                response = session.post(url, data=data, verify=False)
    ...
    def put(self, product_uuid=None, location_uuid=None):
        ...
        try:
            ...
            if args['value']:
                ...
                rel_content[property] ='null' # If 'value' arg is empty, delete the property
    ...
    def get(self, target_uuid):
        ...
        try:
            ...
            if (response.ok):
                ...
                return 'products/'+str(filename),200
            else:
                response_dump = None
                try:
                    response_dump = response.json()
    ...
    def get(self, product_uuid):
        ...
        try:
            ...
            for location in locations:
                return_list.append(location[0]['data'])
            return return_list
...
addtempdeal_argparser = argparser.copy()
@api.route('/<string:product_uuid>/addtempdeal')
class AddLocation(Resource):
    addtempdeal_argparser.add_argument('location_uuid', type=str, required=True, location='form', help='Location node ID')
    addtempdeal_argparser.add_argument('deal_price', type=str, required=True, location='form', help='Deal price')
    addtempdeal_argparser.add_argument('expiry_date', type=str, required=False, location='form', help='Format yyyy-mm-dd')
    addtempdeal_argparser.add_argument('amount', type=str, required=False, location='form', help='how many deals to offer, blank for unlimited')
    @api.expect(addtempdeal_argparser)
    @api.doc(responses={
        200: 'Success',
        400: 'Validation Error'
    })
    def put(self, product_uuid=None):
        '''Product available at location and information about the product'''
        try:
            args = addtempdeal_argparser.parse_args()
            if args['location_uuid']:
                location_uuid = args['location_uuid']
            else:
                return '`location_uuid` is a required field', 400
            if args['deal_price']:
                ...
                return '`deal_price` is a required field', 400
            if args['amount']:
                amount = args['amount']
            else:
                amount = 'unlimited'
            if args['expiry_date']:
                date = datetime.datetime.strptime(args['expiry_date'], "%Y-%m-%d")
            else:
            ...
            relationship_type = 'temp_deal'
            # Create new relationship
            url = neo_http_ap + '/db/data/node/' + str(product_id) + '/relationships'
            data = {'to': neo_bolt_ap + '/db/data/node/' + str(location_id), 'type': relationship_type}
            beacon_id = rel_content['beacon_id']
            code = str(uuid.uuid4()) #shouldn't be uuid, use a short string generator instead
            data['data'] = {
...
readtempdetails_argparser = argparser.copy()
@api.route('/<string:product_uuid>/<string:location_uuid>/gettempdeal')
class Read(Resource):
    @api.expect(readtempdetails_argparser)
    @api.doc(responses={
        200: 'Success',
        400: 'Validation Error'
    })
    def get(self, product_uuid, location_uuid):
        ...
        try:
            url = neo_http_ap + '/db/data/cypher'
            try:
                data = {'query': get_temp_deal_availability, 'params': {'uuidp': str(product_uuid), 'uuidl': str(location_uuid)}}
                data = json.dumps(data, separators=(',', ':'))
                response = session.post(url, data=data, verify=False)
                if (response.ok):
                    rel = response.json()['data'][0][0]['data']
...
readtempdetails_argparser = argparser.copy()
@api.route('/<string:code>/gettempdealbycode')
class Read(Resource):
    @api.expect(readtempdetails_argparser)
    @api.doc(responses={
        200: 'Success',
    ...
    def get(self, code):
        ...
        try:
            url = neo_http_ap + '/db/data/cypher'
            try:
                data = {'query': get_temp_deal_by_code, 'params': {'code':code}}
                data = json.dumps(data, separators=(',', ':'))
                response = session.post(url, data=data, verify=False)
                if (response.ok):
                    rel = response.json()['data'][0][0]['data']
                    puuid  =response.json()['data'][0][1]
                    luuid  =response.json()['data'][0][2]
                    return_list = {
                                    'relationship': rel,
                                    'product_uuid': puuid,
                                    'location_uuid': luuid
...
incrementinterest_argparser = argparser.copy()
@api.route('/<string:product_uuid>/incrementinterest')
class Update_Location(Resource):
    updatelocation_argparser.add_argument('location_uuid', type=str, required=True, location='form',help='Location node ID')
    @api.expect(updatelocation_argparser)
    @api.doc(responses={
    ...
    def put(self, product_uuid=None, location_uuid=None):
        ...
        try:
            ...
            if not (response.ok):
                ...
                return message, 400
            rel_content = response.json()
            counter = int(rel_content['interest_counter'])
            rel_content['interest_counter'] = str(counter + 1)
            data = json.dumps(rel_content, separators=(',', ':'))
            response = session.put(url, data=data, verify=False)
            if (not response.ok):
                try:
...
addtempdealorg_argparser = argparser.copy()
@api.route('/<string:product_uuid>/addtempdealorg')
class AddLocation(Resource):
    addtempdealorg_argparser.add_argument('location_uuid', type=str, required=True, location='form', help='Location node ID')
    addtempdealorg_argparser.add_argument('organization_uuid', type=str, required=True, location='form', help='Location node ID')
    addtempdealorg_argparser.add_argument('deal_price', type=str, required=True, location='form', help='Deal price')
    addtempdealorg_argparser.add_argument('expiry_date', type=str, required=False, location='form', help='Format yyyy-mm-dd')
    addtempdealorg_argparser.add_argument('amount', type=str, required=False, location='form', help='how many deals to offer, blank for unlimited')
    @api.expect(addtempdealorg_argparser)
    @api.doc(responses={
        200: 'Success',
        400: 'Validation Error'
    })
    def put(self, product_uuid=None):
        '''Product available at location and information about the product'''
        try:
            args = addtempdealorg_argparser.parse_args()
            if args['location_uuid']:
                location_uuid = args['location_uuid']
            else:
                return '`location_uuid` is a required field', 400
            ...
            relationship_type = 'temp_deal_org'
            # Create new relationship
            url = neo_http_ap + '/db/data/node/' + str(product_id) + '/relationships'
            data = {'to': neo_bolt_ap + '/db/data/node/' + str(location_id), 'type': relationship_type}
            beacon_id = rel_content['beacon_id']
...
readtempdetailsorg_argparser = argparser.copy()
@api.route('/<string:organization_uuid>/gettempdealorg')
class Read(Resource):
    @api.expect(readtempdetailsorg_argparser)
    @api.doc(responses={
        200: 'Success',
        400: 'Validation Error'
    })
    def get(self, organization_uuid):
        ...
        try:
            url = neo_http_ap + '/db/data/cypher'
            try:
                data = {'query': get_temp_deal_by_org, 'params': {'uuid':organization_uuid}}
                data = json.dumps(data, separators=(',', ':'))
                response = session.post(url, data=data, verify=False)
                if (response.ok):
                    rel = response.json()['data'][0][0]['data']
